 ...
 */
#define pr_fmt(fmt)	"FG-UTIL: %s:%d " fmt, __func__, __LINE__
#include <linux/of.h>
...
bool is_qnovo_en(struct fg_dev *fg)
{
	...
	rc = fg_read_iio_chan(fg, CHARGE_QNOVO_ENABLE, &qn_en);
	if (rc < 0) {
		pr_err("Error can't get CHARGE_QNOVO_ENABLE, rc = %d\n", rc);
	}
...
int fg_read_int_iio_chan(struct iio_channel *iio_chan_list, int chan_id,
			int *val)
{
	int rc;
