...
#define FGPARAM(_id, _addr_word, _addr_byte, _len, _num, _den, _offset,	\
	      _enc, _dec)						\
	[FG_SRAM_##_id] = {						\
...
extern int fg_lerp(const struct fg_pt *pts, size_t tablesize, s32 input,
			s32 *output);
extern bool is_chan_valid(struct fg_dev *fg, enum fg_ext_iio_channels chan);
...
extern int fg_write_iio_chan(struct fg_dev *fg,
			enum fg_ext_iio_channels chan, int val);
extern int fg_read_int_iio_chan(struct iio_channel *iio_chan_list, int chan_id,
			int *val);
#endif
