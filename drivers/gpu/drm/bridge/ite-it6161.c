...
#ifndef DRM_COLOR_FORMAT_YCRCB422
...
static enum drm_mode_status it6161_bridge_mode_valid(
...
static void it6161_mipi_rx_interrupt_reg06_process(struct it6161 *it6161, u8 reg06)
{
	...
	if (reg06 & 0x01) {
		...
		dev_dbg(dev, "PPS M video stable Change Interrupt, %sstable",
				m_video_stable ? "" : "un");

		if (m_video_stable) {
			data_id = it6161_mipi_rx_read(it6161, 0x28);
			dev_dbg(dev, "mipi receive video format: 0x%02x", data_id);
		...
		dev_dbg(dev, "PPS P video stable Change Interrupt, %sstable", p_video_stable ? "" : "un");
		if (p_video_stable) {
			dev_dbg(dev, "PVidStb Change to HIGH");
			mipi_rx_calc_rclk(it6161);
		...
		dev_dbg(dev, "PPS MHSync Error interrupt");
	else if (reg06 & 0x04)
		dev_dbg(dev, "PPS MHDE Error Interrupt");
	else if (reg06 & 0x08)
		dev_dbg(dev, "PPS MVSync Error Interrupt");
	else if (reg06 & 0x20)
		dev_dbg(dev, "PPS PHSync Error Interrupt");
	else if (reg06 & 0x40)
		dev_dbg(dev, "PPS PHDE Error Interrupt");
	else if (reg06 & 0x80)
		dev_dbg(dev, "PPS MVDE Error Interrupt");
}
...
static void it6161_mipi_rx_interrupt_reg07_process(struct it6161 *it6161, u8 reg07)
{
	...
	if (reg07 & 0x01)
		dev_dbg(dev, "PatGen PPGVidStb change interrupt !!!\n");
	else if (reg07 & 0x02)
		dev_dbg(dev, "PPS Data Byte Error Interrupt !!!\n");
	else if (reg07 & 0x04)
		dev_dbg(dev, "PPS CMOff Interrupt !!!\n");
	else if (reg07 & 0x08)
		dev_dbg(dev, "PPS CMOn Interrupt !!!\n");
	else if (reg07 & 0x10)
		dev_dbg(dev, "PPS ShutDone cmd Interrupt !!!\n");
	else if (reg07 & 0x20)
		dev_dbg(dev, "PPS TurnOn Interrupt !!!\n");
	else if (reg07 & 0x40) {
		dev_dbg(dev, "PPS FIFO over read Interrupt !!! tx video statle:%d",
				hdmi_tx_get_video_state(it6161));
		it6161_mipi_rx_set_bits(it6161, 0x07, 0x40, 0x40);
	} else if (reg07 & 0x80) {
		dev_dbg(dev, "PPS FIFO over write Interrupt !!!\n");
...
static void it6161_mipi_rx_interrupt_reg08_process(struct it6161 *it6161, u8 reg08)
{
	...
	if (reg08 & 0x01)
		dev_dbg(dev, "ECC 1-bit Error Interrupt !!!\n");
	else if (reg08 & 0x02)
		dev_dbg(dev, "ECC 2-bit Error Interrupt !!!\n");
	else if (reg08 & 0x04)
		dev_dbg(dev, "LM FIFO Error Interrupt !!!\n");
	else if (reg08 & 0x08)
		dev_dbg(dev, "CRC Error Interrupt !!!\n");
	else if (reg08 & 0x10)
		dev_dbg(dev, "MCLK Off Interrupt !!!\n");
	else if (reg08 & 0x20)
		dev_dbg(dev, "PPI FIFO OverWrite Interrupt !!!\n");
	else if (reg08 & 0x40) {
		it6161_mipi_rx_set_bits(it6161, 0x0b, 0x40, 0x00);

		if ((it6161_mipi_rx_read(it6161, 0xC1) & 0x03) == 0x03)
			dev_dbg(dev, "CRC Fail !!!\n");
...
static irqreturn_t it6161_intp_threaded_handler(int unused, void *data)
{
	...
	dev_dbg(dev, "%s: [MIPI] [REG06 = 0x%x] \n", __func__, mipi_rx_reg06);
	dev_dbg(dev, "%s: [MIPI] [REG07 = 0x%x] \n", __func__, mipi_rx_reg07);
	dev_dbg(dev, "%s: [MIPI] [REG08 = 0x%x] \n", __func__, mipi_rx_reg08);
	dev_dbg(dev, "%s: [MIPI] [REG0D = 0x%x] \n", __func__, mipi_rx_reg0d);
	...
	dev_dbg(dev, "%s: [HDMI] [REG06 = 0x%x] \n", __func__, hdmi_tx_reg06);
	dev_dbg(dev, "%s: [HDMI] [REG07 = 0x%x] \n", __func__, hdmi_tx_reg07);
	dev_dbg(dev, "%s: [HDMI] [REG08 = 0x%x] \n", __func__, hdmi_tx_reg08);
	dev_dbg(dev, "%s: [HDMI] [REG0E = 0x%x] \n", __func__, hdmi_tx_reg0e);
	dev_dbg(dev, "%s: [HDMI] [REG0EE = 0x%x] \n", __func__, hdmi_tx_regee);

	if ((mipi_rx_reg06 != 0) || (mipi_rx_reg07 != 0) || (mipi_rx_reg08 != 0)) {
		dev_dbg(dev, "[MIPI rx ++] reg06: 0x%02x reg07: 0x%02x reg08: 0x%02x reg0d: 0x%02x",
	...
	if ((hdmi_tx_reg06 != 0) || (hdmi_tx_reg07 != 0) || (hdmi_tx_reg08 != 0)) {
		dev_dbg(dev, "[HDMI tx ++] reg06: 0x%02x reg07: 0x%02x reg08: 0x%02x reg0e: 0x%02x regee: 0x%02x",
